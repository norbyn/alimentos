...
class Appunto_auth {
    ...
    public function userHasPermission($permission) {
        if (!$this->logged_in())
            return false;  // user can't have the permission if not logged in 
    ...
    public function hashPassword($password) {
        $hasher = new PasswordHash(
                $this->CI->config->item('hash_logarithm', 'appunto-auth/appunto_auth'), $this->CI->config->item('hash_use_portable', 'appunto-auth/appunto_auth')
        );
    ...
    public function create_security_log($success, $message) {
        $username = $this->CI->session->userdata('username');
        $ip_address = $this->CI->input->ip_address();
        $user_agent = $this->CI->input->user_agent();
        $this->CI->usermodel->record_login_attempt($username, $ip_address, $success, $user_agent, $message);
    }
