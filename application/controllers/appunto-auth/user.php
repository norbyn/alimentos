...
class User extends CI_Controller {
    ...
    public function login() {
        ...
        if ($this->appunto_auth->logged_in())
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'GET') {
            ...
            if (!$this->form_validation->run()) {
                $this->usermodel->record_login_attempt($username, $ip_address, 0, $user_agent, 'Intento de inicio de sesión: ' . validation_errors('[', ']'));
                ...
                if (($user != false) && $this->appunto_auth->checkPassword($pass, $user->password) && $user->active == 1) {
                    $this->usermodel->record_login_attempt($username, $ip_address, 1, $user_agent, 'Sesión iniciada.');
                    $this->usermodel->update_last_login($username, $ip_address);
                    $role = AppaRoleTable::getInstance()->getRole($user->id);
                    ...
                    redirect($this->router->default_controller);
                } else if (($user != false) && $this->appunto_auth->checkPassword($pass, $user->password) && $user->active == 0) {
                    $this->form_validation->set_rules('password', 'lang:appunto_form_password', 'callback__locked_user');
    ...
    public function _locked_user($str) {
        $this->form_validation->set_message('_locked_user', lang('appunto_form_locked_user'));
        return false;
    }
...
//        echo AppaRoleTable::getInstance()->getRole(1);
//    }
}
