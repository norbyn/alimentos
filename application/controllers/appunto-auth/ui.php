...
class Ui extends CI_Controller {
        ...
        switch ($method) {
                        ...
                        foreach ($controller_dirs as $controller_dir) {
                            ...
                            foreach ($controller_files as $filename) {
                                ...
                                if (!class_exists($classname) || ($classname == 'Ui')) {
                                    ...
                                    if ($filename != 'ui.php')
                                        require_once($this_dir . $filename);
            ...
            case 'statusinfo':
                ...
                array_push($info_array, array(
                    'info_item' => 'Versión de Codeigniter',
                    'info_val' => CI_VERSION
                ));
                array_push($info_array, array(
                    'info_item' => 'Versión de Appunto-Auth',
                    'info_val' => $this->config->item('appunto_auth_version', 'appunto-auth/appunto_auth')
                ));

                $hooks_enabled = $this->config->item('enable_hooks');
                array_push($info_array, array(
                    'info_item' => 'Hooks habilitados',
                    'info_val' => ($hooks_enabled ? 'Sí' : 'No'),
                ...
                array_push($info_array, array(
                    ...
                    'info_val' => ($csrf_protection ? 'Sí' : 'No'),
                ...
                array_push($info_array, array(
                    ...
                    'info_val' => ($sessions_using_database ? 'Sí' : 'No'),
                ...
                array_push($info_array, array(
                    ...
                    'info_val' => ($sessions_encrypted ? 'Sí' : 'No'),
                ...
                array_push($info_array, array(
                    ...
                    'info_val' => ($allow_private_without_permission ? 'Sí' : 'No'),
    ...
    function _validate_permission_in_use_by_user($id) {
        if ($this->permissionmodel->in_use_by_user($id)) {
            $this->form_validation->set_message(
                    '_validate_permission_in_use_by_user', $this->lang->line('appunto_permission_in_use_by_user')
            );
            return false;
        }
        return true;
    ...
    function _validate_permission_in_use_by_role($id) {
        if ($this->permissionmodel->in_use_by_role($id)) {
            $this->form_validation->set_message(
                    '_validate_permission_in_use_by_role', $this->lang->line('appunto_permission_in_use_by_role')
            );
            return false;
        }
        return true;
    ...
    function _validate_permission_in_use_by_path($id) {
        if ($this->permissionmodel->in_use_by_path($id)) {
            $this->form_validation->set_message(
                    '_validate_permission_in_use_by_path', $this->lang->line('appunto_permission_in_use_by_path')
            );
            return false;
        }
        return true;
    ...
    function _validate_role_in_use_by_user($id) {
        if ($this->rolemodel->in_use_by_user($id)) {
            $this->form_validation->set_message(
                    '_validate_role_in_use_by_user', $this->lang->line('appunto_role_in_use_by_user')
            );
            return false;
        }
        return true;
    ...
    function _validate_role_in_use_by_permission($id) {
        if ($this->rolemodel->in_use_by_permission($id)) {
            $this->form_validation->set_message(
                    '_validate_role_in_use_by_permission', $this->lang->line('appunto_role_in_use_by_permission')
            );
            return false;
        }
        return true;
